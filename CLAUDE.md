...
**Flujo normal completo, y 2 aclaraciones del usuario sobre PENDING/RETURN (2026-09-23):** el flujo de un pago ACH es `ACH TRANSMITTED → RETURN → PENDING → ACCEPTED/COLLECTED o NOT_COLLECTED`. `PENDING` es el banco reintentando el cobro — nunca es un estado final por sí solo. Dos aclaraciones sobre qué hacer cuando un `NOT_COLLECTED` ya aplicado recibe *después* un reporte de `PENDING` para el mismo cheque:
...
- **Si el `NOT_COLLECTED` se aplicó por un código de retorno definitivo** (la excepción de arriba) y luego llega un `PENDING` 1-3 días después: **no se revierte**. Verificado con 9 casos reales (R02×6, R04×2, R13×1) — el `PENDING` tenía fecha interna de transacción 1-3 días después del `RETURN` y se publicó al día siguiente del reporte de Returns: es el mismo cheque reapareciendo en el snapshot diario rutinario de Check Collection antes de que ese pipeline reflejara el rechazo, no un reintento bancario real (una cuenta cerrada/routing inválido no puede legítimamente volver a "pending").
