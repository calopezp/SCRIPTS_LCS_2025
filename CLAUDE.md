...
- `ACH_REPORTADOS/build_index.py` (paso 3/3 de `run_daily_new_files.sh`, transmisión ACH) tenía este mismo hueco de la Rule 2 sin cubrir hasta 2026-09-15 -- no tenía NINGÚN corte de antigüedad, así que un archivo nunca antes visto se aplicaba solo a Salesforce sin importar qué tan viejo fuera su nombre. Se detectó porque la corrida diaria reportó `ACH_20240619.csv`/`ACH_20260311.csv` (ambos vacíos, sin impacto real, pero confirmó el hueco). Corregido con corte propio de **4 meses (120 días) o año anterior** al actual (más laxo que los 7 días de Returns/Collection porque este pipeline agrupa dos fuentes -- OneDrive + Salesforce Files -- con cadencias distintas): archivo nunca visto y fuera de ese rango se indexa igual (no se pierde del histórico en `transmission_index.csv`) pero NO entra a `index/last_run_delta.csv` (lo que de verdad se aplica a Salesforce) -- queda en su propio `index/archivos_viejos_pendientes_confirmacion.csv` (carpeta `ACH_REPORTADOS/index/`, no confundir con el de `COLLECTIONS/index/` de Returns/Collection) para aplicar manual con `./run_transmission_import.sh "<ruta>" apply`. De paso se corrigió un bug de fondo: un archivo con 0 registros o sin fecha reconocible en el nombre nunca quedaba marcado como "visto" y se re-escaneaba/reportaba en consola en cada corrida futura para siempre -- nuevo `index/scanned_files_log.csv` registra todo archivo examinado sin importar el resultado. Detalle completo en `BITACORA_HALLAZGOS_TECNICOS.md` (fila 2026-09-15).
...
**Excepción confirmada por el usuario (2026-09-22): un código de retorno "definitivo" salta la espera de 26 días.** Si `SM_Return_code__c` ya es uno de los que `SM_ReturnCodeNotifier` clasifica como "nunca se resuelve reintentando el mismo método de pago" (**R02** cuenta cerrada, **R04/R13** cuenta o routing inválido, **R07** autorización revocada, **R10** cliente no autoriza el cobro, **R16** cuenta congelada — los 5 reportes dedicados que ya existen, ver sección 2.1 del skill `/collections`), no hace falta esperar los 26 días completos: se puede aplicar `NOT_COLLECTED` de inmediato porque el resultado ya es seguro. **R01/R09 (NSF) y R08 (pago detenido) NO califican para esta excepción** — son los mismos códigos que `SM_ReturnCodeNotifier` excluye de sus reportes de "no resuelve solo" precisamente porque sí pueden resolverse con un reintento; esos sí deben esperar el plazo completo de la Rule 3. Casos reales que lo confirmaron: 5 de 7 Late Payment Fee (R02×2, R04×2, R13×1) con solo 25 días transmitidos se corrigieron de una vez (los otros 2, R08 y R01/NSF, se dejaron esperando); y un barrido posterior de contratos Cancelled/en cola de cancelación encontró 9 más por R02/R10 sin importar antigüedad (nota: la primera vez que se documentó esta excepción se omitió R10 de la lista por error — corregido aquí).

**Flujo normal completo, y aclaraciones del usuario sobre PENDING/RETURN (2026-09-23):** el flujo de un pago ACH es `ACH TRANSMITTED → RETURN → PENDING → ACCEPTED/COLLECTED o NOT_COLLECTED`. `PENDING` es el banco reintentando el cobro — nunca es un estado final por sí solo. Reglas sobre qué hacer cuando un `NOT_COLLECTED` ya aplicado recibe *después* un reporte de `PENDING` para el mismo cheque:
- **Si el `NOT_COLLECTED` se aplicó por conteo de días (Rule 3, sin ningún reporte real detrás)** y luego llega un `PENDING` real: el reporte manda — se revierte a `PENDING` y se reporta (mismo tratamiento que ya existe para timeouts de 15 días revertidos, ver `notify_timeout_reversal.apex`). Validado 2026-09-23: no se encontró ningún caso real de este escenario en los 184 hallazgos del Objetivo 3 (queda documentado para cuando aparezca uno).
- **Si el `NOT_COLLECTED` se aplicó por un código de retorno definitivo** (la excepción de arriba): el `PENDING` posterior **solo se aplica (revierte a `PENDING`) si a la fecha de hoy NO han pasado más de 21 días desde `SM_Transmission_Date_ACH_File__c`** — pasado ese umbral, se asume que el `PENDING` es ruido del snapshot diario de Check Collection (el mismo cheque reapareciendo antes de que ese pipeline reflejara el rechazo), no un reintento bancario real, y se queda `NOT_COLLECTED`. Verificado 2026-09-23 con 9 casos reales (R02×6, R04×2, R13×1): 8 con 26 días (no aplica, quedan `NOT_COLLECTED`), 1 con 15 días (calificaría por días, pero ver excepción de contrato Cancelled abajo).
- **Excepción: un contrato `Cancelled` nunca revierte a `PENDING`, sin importar los días.** Un contrato Cancelled no debe tener payments en ningún estado de tránsito (`ACH TRANSMITTED`/`PENDING`/`RETURN`) — siempre se resuelven a un estado terminal (`NOT_COLLECTED` o `COLLECTED`), nunca se dejan "pendientes". Caso real: `PY-01899177` (contrato `00316302`, Cancelled, R02, 15 días) calificaba por el umbral de 21 días pero se queda `NOT_COLLECTED` por ser Cancelled.
