...
- `./run_daily_new_files.sh apply` — uso diario normal, los 3 pipelines en un paso, solo archivos nunca antes indexados. En modo `apply` también dispara `notify_r02.apex` y `notify_r10.apex` (ver sección 4 abajo).
...
├── run_daily_new_files.sh      # COMANDO DIARIO -- los 3 pipelines + notify_r02.apex/notify_r10.apex en modo apply, solo archivos nunca antes indexados (ver CLAUDE.md sección 2)
...
- **`notify_r02.apex` / `notify_r10.apex`** (nuevos 2026-09-14): invocan `SM_ReturnCodeNotifier.sendR02Report()` / `.sendR10Report()` — correo con contratos activos que tienen un `SM_Payment__c.SM_Return_code__c` en `R02` (ACCOUNT CLOSED) o `R10` (CUSTOMER ADVISES NOT AUTHORIZED) sin resolver. Ninguno de los 2 se arregla reintentando el mismo método de pago — R02 porque la cuenta ya no existe, R10 porque el cliente ya le dijo al banco que no autoriza el cobro. Excluye contratos `Cancelled`/`Finalized`/`SM_Customer_Cancellation__c=true`; marca "OJO" si hubo un pago `ACCEPTED` después del último return (posible ya resuelto). Se disparan solo en modo `apply` de `run_daily_new_files.sh`. Distinto del CSV de R10 para Comercial (`CheckCollectionImport_R10_ClienteSolicitoDevolucion.csv`, generado por `extract_check_collection.py` por corrida) — ese es seguimiento comercial puntual, este es un correo acumulativo del estado actual en Salesforce.
