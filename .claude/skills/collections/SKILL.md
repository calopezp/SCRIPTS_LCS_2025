...
├── notify_r07.apex              # igual pero para R07 (autorizacion revocada, no un cargo puntual disputado como R10) -- misma clase, ver sección 5
...
- **`UTILITARIOS/mark_transmitted_accepted.apex`**: marca `ACCEPTED`/`COLLECTED` los payments en `Payment_Status__c='ACH TRANSMITTED'` con `SM_Transmission_Date_ACH_File__c` de 15+ días (`DAYS_SINCE_TRANSMITTED=15`, confirmado por Carlos 2026-09-21 como valor definitivo -- coincide con lo validado por Comercial el 2026-09-14; hubo un valor de 20 sin confirmar entre el 2026-09-18 y el 2026-09-21, ya revertido) y sin ningún reporte (`SM_Check_Collection_Status__c = null`). Historial: `"PRC_AUT_<N>D_SIN_REPORTE: ACCEPTED //"`. **Automatizado como paso 4/4 de `run_daily_new_files.sh` el 2026-09-18** (decisión explícita de Carlos) — corre siempre DESPUÉS de los 3 pipelines del mismo script, para que un reporte real que llegó ese mismo día tenga prioridad. El wrapper `UTILITARIOS/run_mark_transmitted_accepted.sh` sigue existiendo para correrlo suelto si hace falta. **Bug de seguridad encontrado y corregido el mismo día**: el archivo tenía `DRY_RUN=false` committeado como default (a diferencia de todo el resto del pipeline, que default a `true`) — correr el wrapper "sin argumentos" (supuestamente solo preview) en realidad ejecutaba el UPDATE real; no tuvo consecuencias porque en ese momento había 0 candidatos reales, pura casualidad. Corregido a `DRY_RUN=true`, ver `BITACORA_HALLAZGOS_TECNICOS.md` fila 2026-09-18.
