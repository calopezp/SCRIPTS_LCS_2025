...
|---|---|---|---|
| 2026-09-14 | 19 contratos activos con ACH Return **R02 (cuenta cerrada)** sin resolver, 78 pagos afectados — reportados por el nuevo `notify_r02.apex` | Decisión de negocio: contactar a cada cliente para actualizar su método de pago (reintentar no sirve, la cuenta ya no existe). El correo diario solo informa, no actúa | Esperando que Carlos revise el correo y decida el proceso de contacto al cliente |
| 2026-09-14 | 13 contratos activos con ACH Return **R10 (cliente no autoriza el cobro)** sin resolver, 17 pagos afectados — reportados por el nuevo `notify_r10.apex` | Decisión de negocio: estos clientes le dijeron al banco directamente que no autorizan el cargo — reintentar sin hablar primero puede generar otra disputa. El correo diario solo informa, no actúa | Esperando que Carlos revise el correo y decida el proceso de contacto al cliente |
| 2026-09-14 | 24 contratos con **R04/R13 (cuenta/routing inválido)**, 8 con **R07 (autorización revocada)**, 2 con **R16 (cuenta congelada)** — reportados por los 3 notify_*.apex nuevos | Decisión de negocio por cada grupo: R04/R13 necesita corregir el dato bancario con el cliente; R07 necesita una autorización nueva; R16 es un tema legal fuera del control del cliente, seguimiento aparte | Esperando que Carlos revise los correos |
| 2026-09-14 | Código **R08 (pago detenido por el cliente)** — 149 pagos en contratos activos, el volumen más alto de todos los códigos "no-NSF" — deliberadamente NO se agregó notificación todavía | Decidir si amerita su propio reporte — a diferencia de R02/R10/R04/R13/R07/R16, un R08 puede ser un stop puntual de un solo pago, no necesariamente de todos los futuros, hace falta más análisis antes de tratarlo igual | Carlos no lo seleccionó en la ronda anterior — queda para revisar después si se quiere profundizar |
| 2026-09-14 | Contrato `00316708`: 6 meses sin cobrar ninguna mensualidad tras el AC inicial ($1,701 en 19 pagos fallidos/pendientes, 3 de ellos R02) | Candidato a la misma revisión de cobranza/cancelación que los 4 contratos ya flagueados (`00270432, 00275989, 00316097, 00315643`) — pendiente que Carlos confirme si se agrega a esa lista | Carlos preguntó si agregarlo — sin respuesta todavía |
