...
|---|---|---|---|
| 2026-09-14 | 4 pagos de la re-auditoría del 2026-09-13 aún sin procesar: `PY-01887711` (00318278, AC $99, `Payment Process`), `PY-01890512` (00317978, AC $119, `Payment Process`), `PY-01890575` (00318365, AC $139, `Payment Process`) → candidatos a `ACCEPTED`/`COLLECTED` (posible activación automática, como pasó con `PY-01887694`); `PY-01809138` (00316708, Fee, `Activated`) → candidato a `NOT_COLLECTED` | Confirmación de Carlos para procesar cada uno (los 3 AC se notifican aparte según su regla, no se agrupan con Fees) | Sin tocar — esperando decisión |
...
| 2026-09-13 | **2 órdenes AC duplicadas** siguen activas — `ACH-29449` (00318054, $139, fecha 31-jul vencida), `ACH-29452` (00317929, $139, fecha 03-ago vencida). `ACH-29448` (00317976) ya no existe (revalidado 2026-09-14, 0 registros con ese nombre — probablemente borrada manualmente, sin registrar cómo) | Decidir si se cancelan (`SM_Payment_Status__c = 'Canceled'`) antes de que el batch diario de cobro (corre Lun-Vie) las recoja e intente cobrar el AC por segunda vez | **Urgente** — esperando confirmación explícita de Carlos |
| 2026-09-14 | `ACH-29453` (Subscription, $139, contrato `00317929`) — creada el mismo domingo 5:57 AM (hora PR) que `ACH-29452`, por la misma identidad `clopez@legal-credit.com`, sin job programado que lo explique (ver hallazgo en la bitácora) | Determinar si es una orden Subscription legítima (efecto colateral de que algo reactivó el contrato) o parte del mismo evento sin explicar — no se ha tocado | Sin investigar más allá de lo ya documentado — origen real sigue desconocido |
