...
|---|---|---|---|
| 2026-09-14 | `PY-01895480` (contrato de Caso 01) — el índice de Returns trae `RETURN`/`R01` (reporte 09/04/2026), pero en Salesforce quedó `PENDING` con `SM_Return_code__c` vacío | Bug real encontrado: `update_check_collection.apex` escribe `SM_Return_code__c = r.returnCode` en cada update — como `collections_index.csv` NO tiene columna de código de retorno, cualquier pago que Check Collection vuelva a tocar **le borra el código** que Returns ya le había puesto. Puede estar afectando el conteo real de los correos R02/R10/R04+R13/R07/R16 (podrían estar subestimados) | Sin corregir — ni el pago puntual ni el bug de fondo. Pendiente decidir alcance antes de tocar el script |
