...
|---|---|---|---|
| 2026-09-13 | 400 `SM_Payment__c` con `SM_Check_Collection_Status__c = 'PENDING'` transmitidos en 2025 (posible subconjunto de los "1,557 ruido histórico" ya cerrados el 2026-09-10) | Carlos quería pasarlos a `NOT_COLLECTED` (+ `Payment_Status__c='REJECTED'` y `SM_Check_Collection__c=true`, que YA tienen en el 100% de la muestra revisada — el único cambio real sería `SM_Check_Collection_Status__c` y el append a `SM_Id_Salesforce_LCS__c`). Se encontró que **137 de los 400 (34%) tienen `SM_Id_Salesforce_LCS__c` empezando con "Collected"** — el sistema legado dice que sí se cobraron, contradice el plan de marcarlos `NOT_COLLECTED` en bloque | Carlos dijo "NO MODIFIQUEMOS NADA" — pendiente su decisión sobre cómo tratar la discrepancia de los 137 antes de retomar. Nada se tocó, todo fue solo lectura |
| 2026-09-13 | Grupo B — 22 contratos con orden ACH Subscription/Monthly cuyo pago quedó confirmado `COLLECTED` por el banco (validado contra índices RETURNS/COLLECTIONS) pero `SM_Check_Collection_Status__c` sigue en `PENDING` en Salesforce | Actualizar el `SM_Payment__c` a `ACCEPTED`/`COLLECTED` (la orden ACH **NO se toca**, ver bitácora 2026-09-13 — marcarla `Completed` sería incorrecto en una orden mensual recurrente). 8 de los 22 contratos están `Cancelled`: `00313481, 00313871, 00315355, 00315765, 00315786, 00315999, 00316490, 00316695`. 14 están `Activated`/`Payment Process`: `00242758, 00309448, 00310716, 00313996, 00315003, 00315161, 00315395, 00315429, 00315550, 00315670, 00316074, 00316384, 00316604, 00316739` | Esperando confirmación de Carlos para procesar (mismo patrón que Grupo A: `SM_TriggerHandler.avoidAllHandlerExcecution=true`) |
| 2026-09-13 | `PY-01812404` (contrato `00316911`) — único pago tipo **AC** entre los 28 confirmados `COLLECTED` por el banco, excluido a pedido explícito de Carlos | Carlos dijo que lo notificáramos y lo revisara él aparte — sin acción tomada todavía | Esperando que Carlos lo revise |
|---|---|---|---|
